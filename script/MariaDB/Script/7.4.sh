...
PLUGINS=$(mariadb -N -B -e "SHOW PLUGINS;" 2>/dev/null | grep -E 'simple_password_check|cracklib_password_check' | grep ACTIVE)

# Ambil variabel password policy
PASSWORD_VARS=$(mariadb -N -B -e "SHOW VARIABLES LIKE '%pass%';" 2>/dev/null)

DETAILS=""

if [[ -z "$PLUGINS" ]]; then
    STATUS="FAIL"
    DETAILS+="Plugin password complexity (simple_password_check atau cracklib_password_check) tidak aktif.\n"
else
    DETAILS+="Plugin aktif:\n$PLUGINS\n"
fi
...
if [[ -z "$MIN_LENGTH" || "$MIN_LENGTH" -lt 14 ]]; then
    STATUS="FAIL"
    DETAILS+="simple_password_check_minimal_length kurang dari 14.\n"
fi

if [[ "$STRICT_VALIDATION" != "ON" ]]; then
    STATUS="FAIL"
    DETAILS+="strict_password_validation tidak diaktifkan.\n"
fi

if [[ -z "$DICTIONARY_FILE" ]]; then
    STATUS="FAIL"
    DETAILS+="cracklib_password_check_dictionary tidak diatur ke file kamus yang sesuai.\n"
else
    DETAILS+="Dictionary file digunakan: $DICTIONARY_FILE\n"
fi

if [[ "$STATUS" == "PASS" ]]; then
    DETAILS+="Semua kebijakan kompleksitas password memenuhi rekomendasi."
fi
