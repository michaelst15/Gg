...
ENCRYPT_VARS=$(mysql -u root -p'bangtob150420' -N -B -e "SELECT VARIABLE_NAME, VARIABLE_VALUE FROM information_schema.global_variables WHERE VARIABLE_NAME LIKE '%ENCRYPT%';" 2>/dev/null)

# Deteksi apakah semua nilai OFF
if echo "$ENCRYPT_VARS" | grep -q "OFF"; then
    STATUS="Fail"
    DETAILS="
Beberapa variabel enkripsi at-rest masih OFF:
$ENCRYPT_VARS"
fi
