...
SECURE_FILE_PRIV=$(mysql -u root -p'bangtob150420' -N -B -e "SHOW GLOBAL VARIABLES WHERE Variable_name = 'secure_file_priv';" 2>/dev/null | awk '{print $2}')
...
if [[ -z "$SECURE_FILE_PRIV" ]]; then
    ...
    if [[ ! -d "$SECURE_FILE_PRIV" ]]; then
        STATUS="Fail"
        DETAILS="
secure_file_priv diset ke path yang tidak ada: 
$SECURE_FILE_PRIV."
    fi
fi
