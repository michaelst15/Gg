...
RELAY_LOG_BASENAME=$(mysql -u root -p'bangtob150420' -N -B -e "SHOW VARIABLES LIKE 'relay_log_basename';" | awk '{print $2}')
...
if [[ -n "$RELAY_LOG_BASENAME" ]]; then
    ...
    if [[ -n "$RELAY_LOG_FILES" ]]; then
        ...
        if [[ -z "$NON_COMPLIANT" ]]; then
            ...
            STATUS="Fail"
            VALUE="
Ditemukan file relay log dengan permission tidak sesuai:
$NON_COMPLIANT"
        fi
    else
        STATUS="Fail"
        VALUE="
Tidak ditemukan file relay log dengan pola: 
${RELAY_LOG_BASENAME}*"
    fi
else
    STATUS="Fail"
