...
SYMLINK_STATUS=$(mysql -u root -p'bangtob150420' -N -B -e "SHOW VARIABLES LIKE 'have_symlink';" 2>/dev/null | awk '{print $2}')

if [[ "$SYMLINK_STATUS" != "DISABLED" ]]; then
    STATUS="Fail"
    DETAILS="
Symbolic links saat ini diaktifkan 
(have_symlink=$SYMLINK_STATUS)."
fi
