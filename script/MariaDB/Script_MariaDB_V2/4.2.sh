...
DB_LIST=$(mysql -u root -p'bangtob150420' -N -B -e "SELECT SCHEMA_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME NOT IN ('mysql','information_schema','sys','performance_schema');" 2>/dev/null)
...
if [[ -z "$DB_LIST" ]]; then
    ...
    STATUS="Fail"
    DETAILS="
Ditemukan database non-standar: 
$DB_LIST
