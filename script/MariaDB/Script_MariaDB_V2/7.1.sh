...
OLD_PASSWORDS=$(mysql -u root -p'bangtob150420' -N -B -e "SHOW VARIABLES WHERE Variable_name = 'old_passwords';" 2>/dev/null | awk '{print $2}')
SECURE_AUTH=$(mysql -u root -p'bangtob150420' -N -B -e "SHOW VARIABLES WHERE Variable_name = 'secure_auth';" 2>/dev/null | awk '{print $2}')
...
if [[ "$STATUS" == "PASS" ]]; then
    DETAILS="
mysql_old_password plugin dinonaktifkan. 
old_passwords = OFF  
secure_auth = ON.
"
fi
