...
if [[ "$PLUGIN_STATUS" == "ACTIVE" ]]; then
    ...
    if [[ -n "$INVALID_USERS" ]]; then
        STATUS="Fail"
        DETAILS="unix_socket plugin active; unauthorized users or non-localhost hosts detected: $INVALID_USERS"
    else
